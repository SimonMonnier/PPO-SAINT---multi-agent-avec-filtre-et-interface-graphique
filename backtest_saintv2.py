...
def add_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # BONUS : features supplémentaires comme dans ton live
...
def compute_entry_atr(df_closed: pd.DataFrame) -> float:
    if "atr_14" not in df_closed.columns or len(df_closed) == 0:
        return 0.0
    atr = float(df_closed["atr_14"].tail(5).mean())
    return max(atr, 0.0)
...
def run_backtest(cfg: LiveConfig):
    ...
    # On démarre à lookback+1 pour construire l'obs sur df[:i]
    for i in range(start_index + 1, n - 1):
        ...
        # ================= FILTRE LONG-ONLY DEMANDE =================
        # Bloque les entrées LONG si close[-1] < close[-3] * 0.9975
        if cfg.side == "long" and len(df_closed_for_obs) >= 3 and state.position == 0:
            last_close = df_closed_for_obs["close"].iloc[-1]
            close_3 = df_closed_for_obs["close"].iloc[-3]
